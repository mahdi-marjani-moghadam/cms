<div class="top-menu">
<div class="customer-menu text-sm justify-between">
        <span class="relative mr-0 ml-auto">قیمت طلا:‌  @convertCurrency(getGoldPrice()['priceToman']) تومان</span>
    ...
    <header class='border-b py-1 px-1 lg:px-5  bg-white font-sans  '>
        <div class='flex flex-wrap items-center lg:gap-y-2 gap-y-4 gap-x-4'>
            ...
            <div class='flex items-center mr-auto lg:order-1'>
                ...
                <a href="/search?in_stock=on"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24"
                        height="24" viewBox="0 0 32 32" style=" fill:#000;">
                        <path
                ...
                @endphp
            <ul id="collapseMenu"
    class="max-lg:px-4 lg:!flex lg:mr-10 lg:gap-x-8 max-lg:gap-y-2 max-lg:hidden max-lg:w-full max-lg:my-4">

    @foreach($menus as $menuItem)

        @if($menuItem->children->count())

            <li class="max-lg:border-b last:border-b-0 max-lg:py-2 md:mb-0 relative parent">

                <a href="{{ url($menuItem->link) }}"
                    class="lg:hover:text-yellow-600 text-gray-600 block font-bold text-[15px] md:ml-5">

                    <i class="arrow top-2.5 rotate-45 left-0 md:left-1 border-gray-400"></i>
                    {{ $menuItem->label }}
                ...
                <ul class="hidden submenu">
                    ...
                    @foreach($menuItem->children as $subMenu)
                        ...
                        @if($subMenu->children->count())
                            ...
                            <li class="relative parent  max-lg:border-b last:border-b-0">

                                <a href="{{ in_array($subMenu->type,['internal','external'])
                                        ? url($subMenu->link)
                                        : '/#'.$subMenu->link }}"
                                    class="lg:hover:text-yellow-600 text-gray-600 block  text-[15px] py-2 px-10">
                                    <i class="arrow rotate-45 md:rotate-135 left-0 top-5 md:left-5 border-gray-400"></i>
                                    {{ $subMenu->label }}

                                </a>

                                {{-- level 3 --}}
                                <ul class="hidden submenu submenu-level-2 lg:top-0 lg:right-full">

                                    @foreach($subMenu->children as $subMenu2)

                                        <li>

                                            <a href="{{ in_array($subMenu2->type,['internal','external'])
                                                    ? url($subMenu2->link)
                                                    : '/#'.$subMenu2->link }}"
                                                class="lg:hover:text-yellow-600 text-gray-600 block  text-[15px] py-2 px-10">
                        ...
                        @else
                            ...
                            <li class="max-lg:border-b last:border-b-0">
        ...
        @else
            ...
            <li class="max-lg:border-b max-lg:py-2 md:mb-0">
                ...
                <a href="{{ in_array($menuItem->type,['internal','external'])
                        ? url($menuItem->link)
                        : '/#'.$menuItem->link }}"
                    class="lg:hover:text-yellow-600 text-gray-600 block font-bold text-[15px]">
    ...
    <script src="{{ url('/dinggold/jquery-3.6.0.min.js') }}"
