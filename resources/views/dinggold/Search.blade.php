...
@section('Content')
    ...
    */ @endphp
    ...
    <section class="mb-0 py-0">
        <div class="flex one two-500 three-800 center">
            ...
            <form action="{{ route('search') }}" class="flex ">
                ...
                <div class="flex gap-1 !w-24 items-center">
                    موجود
                    <input type="checkbox" class="p-0 m-0 h-5 w-5"  name="in_stock"  {{ app('request')->in_stock == 'on'? 'checked':'' }}>
                </div>

                <div class="flex center !w-auto items-center">


                    <input name="q" class="px-4 m-1 h-11 rounded-r" alt="جستجو" type="text" value="{{ app('request')->q }}"
                        placeholder="جستجوی محصول / محتوا / کمپانی" required>


                    <button class="bg-gray-700 !border-y-2 !border-gray-700 !rounded-r-none !m-0 text-white h-full"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
    ...
    <section class="bg-gray search-items my-0">
        <div class="flex one ">
            <h2>محصولات</h2>
            <div>
                @if (count($products))
                    <div class="grid grid-cols-2 gap-2 md:grid-cols-5  ">
                        ...
                        @foreach ($products as $content)
                            <a href="{{ $content->slug }}" class="">
                                <div class="shadow hover p-0 border-radius-5 text-center">
                                    ...
                                    <div class="py-0 px-1 m-0 ">
                                        <div class="truncate">{{ $content->title }}</div>
