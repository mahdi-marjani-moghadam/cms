    ...
    <div class="list">
        ...
        <div class="flex">
            ...
            <div class="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="">
                            <div class="mr-0">
                                <a target="__blank" href="{{ url($pr->slug) }}">
                                    ...
                                    <img class="rounded" src="{{ $content['attributes']['image'] }}" alt="">
                                    @else
                                    <img class="m-auto p-4" width="" src="https://img.icons8.com/ios/50/cccccc/no-image.png" alt="company-no-image" />
                                    @endif
                                </a>
                            </div>
                            ...
                            <div class="pr-5">
                                ...
                                @if (isset($pr['attr']['in-stock']) && $pr['attr']['in-stock'] == 1)
                                <p class="flex justify-start"><svg class="w-10 fill-lime-950" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" class="nu rw uk axs">
                ...
                <div class=" pb-3 px-3 bg-slate-100  border rounded">
                    <meta name="_token" content="{{ csrf_token() }}">
                    <h2> آدرس و مشخصات تحویل گیرنده</h2>
                    <div class=" ">موبایل:
                        <span class="    inline-block m-1 rounded " data-field="mobile" data-label="@lang('messages.mobile')">{{ Auth::user()->customer->mobile ?? '' }}</span>
                    </div>
                    <div class="relative">
                        @lang('messages.name'):
                        ...
                        <span class="absolute right-0 top-6 text-red-700  p-1 -mr-3 px-3 text-xs">نام را وارد
                            نمایید</span>
                        @endif
                        <span class="text-editor text-blue-500  px-3 inline-block m-1 mb-4 rounded cursor-pointer" data-field="name" data-label="@lang('messages.name')">{{ Auth::user()->customer->name ?? '' }}</span>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <span class="absolute right-0 top-6 text-red-700  p-1 -mr-3 px-3 text-xs">آدرس را وارد
                            نمایید</span>
                        @endif
                        <span class="text-editor text-blue-500  px-3 inline-block m-1 mb-4   rounded cursor-pointer" data-field="address" data-label="@lang('messages.address')">{{ Auth::user()->customer->address ?? '' }}</span>
                    </div>
                    ...
                    <div class="relative">@lang('messages.zipcode'):
                        ...
                        <span class="absolute right-0 top-6 text-red-700  p-1 -mr-3 px-3 text-xs">کد پستی را وارد
                            نمایید</span>
                        @endif
                        <span class=" text-editor text-blue-500  px-3 inline-block m-1   rounded cursor-pointer" data-field="zipcode" data-label="@lang('messages.zipcode')">{{ Auth::user()->customer->zipcode ?? '' }}</span>
                    </div>
                ...
                <div class="   p-3 bg-slate-100 border rounded">
                    <h2>روش پرداخت</h2>
                    ...
                    <form method="post" id="upload-bill" enctype="multipart/form-data" class="flex mt-6   px-1 py-1 align-" action="{{ route('customer.uploadBill', ['order' => $content->order->id]) }}">
                        @csrf
                        @method('post')

                        <input type="file" name="bill" class="block text-sm text-gray-500
                            border-0 w-2/3
                    ...
                    <p class=" border bg-red-700 text-white text-center rounded-md p-2 mt-1 text-xl">
                        لطفا قبل از ثبت فیش اطلاعات تحویل گیرنده را کامل بفرمایید.
                    </p>
                    @else
                        ...
                        <form>
                            <div class="col-xs-12 py-1 ">
                                <div class="col-md-12 col-xs-12 flex gap-x-3 px-3 my-5">
                                    ...
                                    <input type="text" name="" class=" focus:ring-gray-500 appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                </div>
                            </div>
                            <div class="col-md-12 col-xs-12">
                                {{-- <input type="submit" class="btn btn-info" value="@lang('messages.edit')"> --}}
                                <button class="rounded border bg-blue-500 text-white p-1 px-3 font-normal">تایید
                                    <label class="" for=""></label></button>
