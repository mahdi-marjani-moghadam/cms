                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a target="__blank" href="{{ url(\App\Models\Content::find($item->attributes['product_id'])->slug) }}">
                        ...
                        <td class=""><div style="font-weight:bold">@convertCurrency($item->price) @lang('messages.toman') {{ $item->count }} عدد</div></td>
                        <td>
                            @if ($item->status == 2)
                            ...
                            <div class="">
                                <div class="">
                                    <form class="pull-right" action="{{ route('admin.order.destroy', $item) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button onclick="return confirm('@lang('messages.Are you sure?')')" class="font-full-plus-half-em text-danger btn-xs  no-border no-bg no-padding" type="submit" title="@lang('messages.delete')">
